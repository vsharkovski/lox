...
class AstPrinterMultiline : Stmt.Visitor<String>, Expr.Visitor<String> {
    ...
    private fun printIndented(stmt: Stmt): String = print(stmt).prependIndent(INDENTATION)

    private fun printIndented(expr: Expr): String = print(expr).prependIndent(INDENTATION)

    override fun visitBlockStmt(stmt: Stmt.Block): String =
        "Block:${stmt.statements.fold("") { result, curr -> "$result\n${printIndented(curr)}" }}"

    override fun visitBreakStmt(stmt: Stmt.Break): String = "Break"

    override fun visitClassStmt(stmt: Stmt.Class): String = buildString {
        append("Class: ${stmt.name.lexeme}")
        stmt.superclass?.let { append("\n(superclass)\n${printIndented(it)}") }
        append("\n(methods)\n${stmt.methods.fold("") { result, curr -> "$result\n${printIndented(curr)}" }}")
    }
    ...
    override fun visitLiteralExpr(expr: Expr.Literal): String =
        "Literal: ${expr.value?.toString() ?: "nil"}"
    ...
    override fun visitSuperExpr(expr: Expr.Super): String =
        "Super: method: ${expr.method.lexeme}"
